...
SCENARIO("Lexer is fed source code") {
    ...
    std::vector<std::vector<lexer::Token>> result{
        {
            {       TokenType::If, 1,  0,   "if"}, {TokenType::Identifier, 1,  3, "sausage"}, 
            {TokenType::LoopBegin, 1, 11, "loop"}, {       TokenType::End, 1, 16,     "end"},
            {       TokenType::Is, 1, 20,   "is"}, {TokenType::Identifier, 1, 23,    "near"},
        },
        {
            {       TokenType::If, 1,  0,   "if"}, {TokenType::Identifier, 1,  3, "sausage"}, 
            {TokenType::LoopBegin, 1, 11, "loop"}, {   TokenType::NewLine, 1, 15,      "\n"},
            {  TokenType::VarDecl, 2,  4,  "var"}, {       TokenType::End, 2,  8,     "end"},
            {       TokenType::Is, 2, 12,   "is"}, {TokenType::Identifier, 2, 15,    "near"},
        ...
        {
            {        TokenType::If, 1,  0,   "if"}, { TokenType::ParenOpen, 1,  3,  "("}, 
            {TokenType::Identifier, 1,  4,    "x"}, {     TokenType::AddOp, 1,  5,  "+"},
            {TokenType::Identifier, 1,  6,    "y"}, {TokenType::ParenClose, 1,  7,  ")"},
            {   TokenType::LeqComp, 1,  8,   "<="}, {TokenType::Identifier, 1, 10,  "z"},
            {      TokenType::Then, 1, 12, "then"}, {   TokenType::NewLine, 1, 16, "\n"},
        },
        {
            {TokenType::Identifier, 1,  0, "TokenType"}, {     TokenType::Colon, 1,  9,     ":"},
            {     TokenType::Colon, 1, 10,         ":"}, {TokenType::Identifier, 1, 11, "Colon"},
        ...
        {
            {        TokenType::If, 1,  0,   "if"}, { TokenType::RealLiteral, 1,  3,    "0.0"}, 
            {   TokenType::NeqComp, 1,  7,   "/="}, { TokenType::RealLiteral, 1, 10, "42.424"},
            {      TokenType::Then, 1, 17, "then"}, {     TokenType::NewLine, 1, 21,     "\n"},
            {TokenType::Identifier, 2,  4, "g_od"}, {TokenType::AssignmentOp, 2,  9,     ":="},
